...
public class PredictPHValue {
	...
	public static List<Double> predict(String path){
		...
		try {
			...
			//预测参数位置
			List<Double> targetFactor =tp.getTargetFactor(testSet, 2);
			List<Double> result = new ArrayList<>();
	        for(int i=0;i<3;i++){
	        	List<Double> predict = PredictPHValue.predictPHValue(i,setMap);
			...
			inversedList = Normalization.inverse(fusedList, normalizedArray, 2);
    ...
    private static double[][] reConstructTrainingSet(double[][] testSet,double[][] trainingSet,Object[] originResult){
    ...
    private static double[][] getPredictFators(Map<String,List<List<Parameters>>> setMap,int setSerialNumber,String type){
        double[][] result = null;
